...
class TopickModel {
    ...
    //    if(\db\Database::getInstance()->affectedRows() > 0) {
    //        return true;
    //    }
    //    return false;
    }
    ...
    public static function readWithOpinion($arguments = array()){
        ...
        $id     = $arguments['$id'];
        $limit  = isset($arguments['limit']) ? $arguments['limit'] : 3;
        $offset = isset($arguments['offset']) ? $arguments['offset'] : 0;
        
        $query = "SELECT a.id AS topick_id, a.title, b.id AS opinion_id, b.content, b.author "
                . "FROM public_board.topicks a, public_board.opinions b "
                . "WHERE a.id = b.topick_id "
                . "AND a.id = $id LIMIT $offset,$limit";
        \db\Database::getInstance()->query($query);
        return\db\Database::getInstance()->fetchCollection();
    }
